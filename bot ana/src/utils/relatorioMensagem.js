...
const { agruparErrosPorContaMarketplace, detalharCustos } = require('./relatorioMensagemHelpers');
...
// Gera mensagens detalhadas para WhatsApp agrupando por conta/marketplace

// Gera mensagens detalhadas para WhatsApp agrupando por conta/marketplace, com design aprimorado

function gerarMensagensRelatorioDetalhado(erros, titulo = '', intro = '') {
  const grupos = agruparErrosPorContaMarketplace(erros);
  const mensagens = [];
  if (titulo) mensagens.push(`*━━━━━━━━━━━ ${titulo.toUpperCase()} ━━━━━━━━━━━*`);
  if (intro) mensagens.push(`_${intro}_`);
  Object.entries(grupos).forEach(([chave, lista]) => {
    let msg = `\n*Conta/Marketplace:* _${chave}_\n`;
    msg += '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n';
    msg += `*#* | *Responsável* | *Motivo* | *Conta* | *Venda* | *SKU* | *Desconto* | *Reembolso* | *Devolução* | *Reembolso Parcial* | *Envio* | *Tampo* | *Outro* | *Custo Total* | *Data*\n`;
    msg += '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n';
    lista.forEach((e, i) => {
      const custos = detalharCustos(e.custos || '');
      msg += `${i+1} | ${e.responsavel || '-'} | ${e.motivo || '-'} | ${e.conta || '-'} | ${e.id_venda || '-'} | ${e.sku || '-'} | ${custos['Desconto']} | ${custos['Reembolso']} | ${custos['Devolução']} | ${custos['Reembolso Parcial']} | ${custos['Envio']} | ${custos['Tampo']} | ${custos['Outro']} | ${e.custo_total || '-'} | ${e.data ? formatarDataHora(e.data) : '-'}`;
      msg += '\n';
    });
    msg += '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n';
    // Resumo por categoria de custos
    const { somarCustosPorCategoria } = require('./relatorioMensagemHelpers');
    const totais = somarCustosPorCategoria(lista);
    msg += `*Totais do grupo:*\n`;
    Object.entries(totais).forEach(([cat, val]) => {
      msg += `• ${cat}: R$ ${val.toFixed(2)}\n`;
    });
    mensagens.push(msg);
  });
  return mensagens;
...
// Função auxiliar para formatar data/hora de forma amigável
function formatarDataHora(dataStr) {
  // Aceita ISO, yyyy-MM-dd, dd/MM/yyyy, etc
  let d;
  if (!dataStr) return '-';
...
module.exports = { gerarPDF, enviarEmail, gerarExcelAvancado, gerarPDFDetalhado, gerarMensagensRelatorioDetalhado };
