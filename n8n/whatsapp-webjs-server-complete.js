// =========================
// 🚪 UTILITÁRIO DE ENCERRAMENTO DE CONVERSA
// =========================
/**
 * Verifica se a mensagem é de encerramento de conversa (ex: "sair", "encerrar", "finalizar", etc)
 * Se sim, encerra a sessão do usuário, envia mensagem de agradecimento e retorna true.
 * Caso contrário, retorna false.
 */
async function checkEndConversation(message, userState) {
    const endKeywords = [
        'sair', 'encerrar', 'finalizar', 'fechar', 'parar', 'cancelar', 'terminar', 'encerra', 'fim', 'tchau', 'adeus', 'obrigado', 'obrigada'
    ];
    const body = (message.body || '').toLowerCase().trim();
    if (endKeywords.some(k => body === k || body.startsWith(k + ' '))) {
        if (typeof stateManager !== 'undefined' && stateManager.deleteUserState) {
            await stateManager.setUserState(message.from, {
                step: 'conversation_completed',
                data: userState?.data || {}
            });
        }
        if (message.reply) {
            await message.reply('🙏 Obrigado por conversar conosco! Se precisar de algo, é só enviar uma nova mensagem. Até logo! 👋');
        }
        return true;
    }
    return false;
...
function saveAudioTranscriptForDashboard(params) {
    ...
    if (fs.existsSync(DASHBOARD_AUDIO_FILE)) {
        try {
            const fileContent = fs.readFileSync(DASHBOARD_AUDIO_FILE, 'utf8');
            all = JSON.parse(fileContent);
            if (!Array.isArray(all)) all = [];
        } catch (e) {
            console.error('Erro ao ler/transcrever audio-transcripts.json:', e.message);
            // Backup do arquivo corrompido
            try {
                const backupPath = `./audio-transcripts-backup-${Date.now()}.json`;
                fs.writeFileSync(backupPath, fs.readFileSync(DASHBOARD_AUDIO_FILE));
                console.warn('Backup do arquivo corrompido salvo em:', backupPath);
            } catch (backupErr) {
                console.error('Falha ao criar backup do arquivo corrompido:', backupErr.message);
            }
            all = [];
        }
    }

    // Evita duplicidade (mesmo chatId, data e transcript)
    const isDuplicate = all.some(item =>
        item.chatId === safeParams.chatId &&
        item.data === safeParams.data &&
        item.transcript === safeParams.transcript
    );
    if (!isDuplicate) {
        all.unshift(safeParams);
    }
    ...
    try {
        ...
        console.error('Erro ao salvar transcrição de áudio:', err.message);
    }

    // Envio para API REST da dashboard (opcional)
    if (process.env.DASHBOARD_API_URL) {
        axios.post(process.env.DASHBOARD_API_URL, safeParams)
            .then(() => {
                if (process.env.LOG_DASHBOARD_API === 'true') {
                    console.log('Transcrição enviada para dashboard:', safeParams.chatId, safeParams.data);
                }
            })
            .catch(err => {
                console.error('Erro ao enviar transcrição para dashboard:', err.message);
                // Opcional: salvar fila de reenvio offline
                try {
                    const failedPath = './audio-transcripts-failed.json';
                    let failed = [];
                    if (fs.existsSync(failedPath)) {
                        failed = JSON.parse(fs.readFileSync(failedPath, 'utf8'));
                        if (!Array.isArray(failed)) failed = [];
                    }
                    failed.unshift({ ...safeParams, error: err.message, failedAt: new Date().toISOString() });
                    if (failed.length > 200) failed = failed.slice(0, 200);
                    fs.writeFileSync(failedPath, JSON.stringify(failed, null, 2));
                } catch (failSaveErr) {
                    console.error('Falha ao registrar falha de envio para dashboard:', failSaveErr.message);
                }
            });
    }
}
...
// Importações necessárias (movidas para o topo)
...
// Instanciar gerenciador de banco de dados e debouncer
let messageDebouncer = null;
if (typeof dbManager !== 'undefined' && dbManager !== null) {
    messageDebouncer = new MessageDebouncer(dbManager, {
        waitTime: 3000, // 3 segundos
        maxMessages: 5,
        maxWaitTime: 10000
    });
    console.log('✅ MessageDebouncer inicializado');
}
...
// Função para feedback de satisfação
async function askSatisfactionFeedback(client, chatId, context = '') {
    try {
        ...
        console.error('Erro ao enviar feedback de satisfação:', error.message);
    }
}

// Função principal: processa comandos de atendente
async function processAttendantCommands(message) {
    try {
        ...
        if (body.startsWith('#ASSUMIR') || body.toLowerCase().startsWith('/assumir')) {
            ...
            // Verificar se botControl está disponível
            if (!botControl || !botControl.pausedChats) {
                console.error('❌ botControl não está disponível');
                if (message.reply) await message.reply('❌ Erro no sistema de controle. Tente novamente.');
                return true;
            }
            ...
            if (typeof botControl.savePausedChats === 'function') {
            ...
            if (message.reply) {
                await message.reply('✅ Você assumiu este chat. O bot foi pausado para este cliente.');
            }
            return true;
        }
        ...
        if (body.startsWith('#FINALIZAR') || body.toLowerCase().startsWith('/finalizar')) {
            ...
            if (botControl.pausedChats.has(chatId)) {
                const pausedData = botControl.pausedChats.get(chatId);
                const attendantId = pausedData?.attendantId || 'atendente';
                ...
                // Enviar feedback de satisfação
                if (globalThis.client) {
                    await askSatisfactionFeedback(globalThis.client, chatId, 'atendimento humano');
                }
                
                if (message.reply) {
                    await message.reply('✅ Chat finalizado. O bot foi reativado para este cliente.');
                }
            } else {
                if (message.reply) {
                    await message.reply('⚠️ Este chat não estava pausado.');
                }
            }
            return true;
        }
        ...
        console.error('❌ Erro ao processar comando de atendente:', error.message);
        if (message.reply) {
...
// =========================
// 🔄 LISTENER PRINCIPAL DE MENSAGENS (INTEGRAÇÃO COMPLETA)
// =========================

// Função para configurar o listener principal
function setupMessageListener(client) {
    if (!client) {
        console.error('❌ Cliente WhatsApp não fornecido para setupMessageListener');
        return;
    }
    ...
    client.on('message', async (message) => {
        try {
            ...
            if (await processAttendantCommands(message)) {
                return; // Comando processado, não continuar
            }

            // 2. Ignora mensagens de chats pausados
            if (await shouldIgnoreMessage(message)) {
                return; // Chat pausado, não processar
            }

            // 3. Filtro de mensagens (grupos, status, privados, etc)
            if (typeof messageFilter !== 'undefined' && messageFilter.shouldProcessMessage) {
                if (!messageFilter.shouldProcessMessage(message)) {
                    if (FILTER_CONFIG?.logBlockedMessages) {
                        console.log(`🚫 Mensagem bloqueada pelo filtro: ${message.from} | ${message.body || '[sem texto]'}`);
                    }
                    return;
                }
            }

            // 4. Log de mensagens recebidas
            if (typeof addToMessageLog === 'function') {
                addToMessageLog({
                    from: message.from,
                    body: message.body,
                    type: message.type,
            ...
            // 5. Carregar/atualizar estado do usuário
            let userState = null;
            if (typeof stateManager !== 'undefined' && stateManager.getUserState) {
                userState = await stateManager.getUserState(message.from);
                if (!userState) {
                    userState = { step: 'start', data: {}, lastInteraction: Date.now() };
                    await stateManager.setUserState(message.from, userState);
                }
            }

            // 6. Encerramento de conversa (palavras-chave)
            if (typeof checkEndConversation === 'function') {
                if (await checkEndConversation(message, userState)) {
                    if (stateManager?.deleteUserState) {
                        await stateManager.deleteUserState(message.from);
                    }
                    return;
                }
            }

            // 7. Processamento de mensagens de áudio (transcrição, dashboard, etc)
            if (message.type === 'audio' || message.type === 'ptt') {
                if (typeof ConversationFlow !== 'undefined' && ConversationFlow.handleAudioMessage) {
                    const result = await ConversationFlow.handleAudioMessage(message, userState);
                    if (result && stateManager?.setUserState) {
                        await stateManager.setUserState(message.from, {
                            step: result.newStep || userState?.step || 'start',
                            data: result.data || userState?.data || {}
                        });
                    }
                    if (result?.response) {
            ...
            // 8. Processamento de comandos especiais (ex: /help, /start, etc)
            if (message.body && message.body.startsWith('/')) {
                if (message.body.toLowerCase().startsWith('/help')) {
                    ...
                    return;
                } else if (message.body.toLowerCase().startsWith('/start') || message.body.toLowerCase().startsWith('/menu')) {
                    // Resetar estado do usuário
                    if (stateManager?.setUserState) {
                        await stateManager.setUserState(message.from, {
                            step: 'start',
                            data: {}
                        });
            ...
            // 9. DEBOUNCE: Agrupar mensagens antes de processar o fluxo
            if (messageDebouncer) {
                messageDebouncer.addMessage(message, async (combinedMessage) => {
                    const { intentAnalysis, body, from } = combinedMessage;
                    let userStateDebounce = null;
                    if (typeof stateManager !== 'undefined' && stateManager.getUserState) {
                        userStateDebounce = await stateManager.getUserState(from);
                    }
                    if (typeof ConversationFlow !== 'undefined' && ConversationFlow.processMessage) {
                        // Roteamento por intenção
                        let result = null;
                        switch (intentAnalysis.suggestedFlow) {
                            case 'catalog_flow':
                                if (ConversationFlow.processCatalogFlow) {
                                    result = await ConversationFlow.processCatalogFlow(combinedMessage, userStateDebounce);
                                } else {
                                    result = await ConversationFlow.processMessage(combinedMessage, userStateDebounce);
                                }
                                break;
                            case 'support_flow':
                                if (ConversationFlow.processSupportFlow) {
                                    result = await ConversationFlow.processSupportFlow(combinedMessage, userStateDebounce);
                                } else {
                                    result = await ConversationFlow.processMessage(combinedMessage, userStateDebounce);
                                }
                                break;
                            case 'payment_flow':
                                if (ConversationFlow.processPaymentFlow) {
                                    result = await ConversationFlow.processPaymentFlow(combinedMessage, userStateDebounce);
                                } else {
                                    result = await ConversationFlow.processMessage(combinedMessage, userStateDebounce);
                                }
                                break;
                            case 'delivery_flow':
                                if (ConversationFlow.processDeliveryFlow) {
                                    result = await ConversationFlow.processDeliveryFlow(combinedMessage, userStateDebounce);
                                } else {
                                    result = await ConversationFlow.processMessage(combinedMessage, userStateDebounce);
                                }
                                break;
                            case 'human_handoff':
                                if (ConversationFlow.processHumanHandoff) {
                                    result = await ConversationFlow.processHumanHandoff(combinedMessage, userStateDebounce);
                                } else {
                                    result = await ConversationFlow.processMessage(combinedMessage, userStateDebounce);
                                }
                                break;
                            case 'cancellation_flow':
                                if (ConversationFlow.processCancellationFlow) {
                                    result = await ConversationFlow.processCancellationFlow(combinedMessage, userStateDebounce);
                                } else {
                                    result = await ConversationFlow.processMessage(combinedMessage, userStateDebounce);
                                }
                                break;
                        ...
                        // Atualizar estado do usuário após processamento
                        if (result && stateManager?.setUserState) {
                            await stateManager.setUserState(from, {
                                step: result.newStep || userStateDebounce?.step || 'start',
                                data: result.data || userStateDebounce?.data || {}
                            });
                        }
                        // Enviar resposta
                        if (result?.response) {
                            await client.sendMessage(from, result.response);
                        }
                        // Finalizar sessão se solicitado
                        if (result?.finalizeSession && stateManager?.deleteUserState) {
                            await stateManager.deleteUserState(from);
                        }
                        // Encaminhar para atendente se necessário
                        if (result?.forwardToAttendant) {
                            console.log(`➡️ Encaminhando para atendente: ${from}`);
                            // Aqui você pode implementar a lógica de encaminhamento
                        }
                    } else {
                        // Fallback: apenas logar
                        console.log('Mensagem agrupada recebida:', combinedMessage);
                    }
                });
                return;
            }

            // 10. Feedback instantâneo após etapas importantes
            if (userState?.step === 'finalizacao' && typeof sendInstantFeedback === 'function') {
                await sendInstantFeedback(client, message.from, 'rating');
            }

            // 11. Envio para n8n (webhook) - opcional
            if (N8N_WEBHOOK_URL && process.env.SEND_TO_N8N === 'true') {
                try {
                    ...
                    console.error('❌ Erro ao enviar mensagem para n8n:', err.message);
                }
            }

            // 12. Atualizar métricas do sistema
            if (typeof SystemMonitor !== 'undefined' && SystemMonitor.updateMetrics) {
                SystemMonitor.updateMetrics('message_processed');
            }

        } catch (err) {
            // Logging de erro robusto
            ...
            try {
                await client.sendMessage(message.from, '⚠️ Ocorreu um erro ao processar sua mensagem. Tente novamente mais tarde ou digite "atendente" para falar com uma pessoa.');
            } catch (replyError) {
...
if (typeof globalThis !== 'undefined') {
    globalThis.processAttendantCommands = processAttendantCommands;
    globalThis.shouldIgnoreMessage = shouldIgnoreMessage;
    globalThis.askSatisfactionFeedback = askSatisfactionFeedback;
    globalThis.setupMessageListener = setupMessageListener;
    globalThis.logBotControlAction = logBotControlAction;
}

// =========================
// 🔄 INICIALIZAÇÃO AUTOMÁTICA (se cliente global existir)
// =========================

// Se o cliente já existe globalmente, configurar listener automaticamente
if (typeof globalThis !== 'undefined' && globalThis.client) {
    setupMessageListener(globalThis.client);
} else if (typeof module !== 'undefined' && module.exports) {
    // Exportar para uso em outros módulos
    module.exports = {
        processAttendantCommands,
        shouldIgnoreMessage,
        askSatisfactionFeedback,
...
/**
 * Comandos para uso exclusivo de atendentes humanos:
 * - /assumir ou #ASSUMIR: Pausa o bot para o chat e registra o atendente responsável.
 * - /finalizar ou #FINALIZAR: Retoma o bot para o chat e solicita feedback ao cliente.
 *
 * O comando só é aceito se enviado por um atendente autorizado (pode ser expandido para checagem de permissões).
 * O controle é persistente e logado para auditoria.
 */
async function processAttendantCommands(message) {
    try {
        const chatId = message.from;
        ...
        if (body.startsWith('#FINALIZAR') || body.toLowerCase().startsWith('/finalizar')) {
            ...
            if (botControl.pausedChats.has(chatId)) {
                ...
                logBotControlAction('resume', chatId, attendantId, { reason: 'Atendente finalizou o chat' });
                if (globalThis.client && typeof askSatisfactionFeedback === 'function') {
                    await askSatisfactionFeedback(globalThis.client, chatId, 'atendimento humano');
                }
                if (message.reply) await message.reply('✅ Chat finalizado. O bot foi reativado para este cliente.');
            } else {
                if (message.reply) await message.reply('⚠️ Este chat não estava pausado.');
            }
            return true;
        }
        return false;
    } catch (error) {
        if (message.reply) await message.reply('❌ Erro ao processar comando. Tente novamente.');
        return true;
    }
}
...
// Listener real do bot (caso não esteja usando o setupMessageListener global)
// Listener real do bot removido daqui para evitar ReferenceError de client antes da inicialização.
// O listener principal já é configurado corretamente após a criação do client (ver setupMessageListener e inicialização do client).
...
// N8N_WEBHOOK_URL já foi declarado anteriormente, não repetir aqui
 ...
 */
    ...
    static async processMessage(message, userState) {
        ...
        switch (currentStep) {
            ...
            // Novo: fluxo pós-resposta de dúvidas
            case 'faq_post_answer': {
                ...
                if (answer === '2' || answer === 'não' || answer === 'nao' || answer === 'n') {
                    // Dispara o fluxo de satisfação
                    setTimeout(async () => {
                        if (globalThis.client && typeof askSatisfactionFeedback === 'function') {
                            await askSatisfactionFeedback(globalThis.client, message.from, 'atendimento');
                        }
                    }, 1000);
